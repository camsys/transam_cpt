...
describe "planning/_swimlane_detail.html.haml", :type => :view do
  it 'swimlane', :skip do
    ...
    render 'planning/swimlane_detail', :ali => test_ali, :year => Date.today.year

    expect(rendered).to have_link('Update ALI cost')
    expect(rendered).to have_link('Assign Funds')
    expect(rendered).to have_link('Move ALI to Next FY')
    expect(rendered).to have_link('Move ALI to Previous FY')

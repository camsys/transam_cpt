...
class ActivityLineItem < ActiveRecord::Base
  ...
  def is_agency_planning_complete?
    CapitalPlanAction.joins([capital_plan: :organization, capital_plan_action_type: :capital_plan_module_type]).where('organizations.id = ? AND capital_plans.fy_year = ? AND capital_plan_action_types.name = "Agency Approval" AND capital_plan_module_types.name = "Constrained Plan"', capital_project.organization_id, current_planning_year_year).first.try(:completed?)
  end
  ...
  def after_update_callback
    ...
    update_columns(estimated_cost: total_asset_cost) if self.saved_change_to_capital_project_id? || self.saved_change_to_fy_year?
  end

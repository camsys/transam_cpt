...
class CapitalProjectBuilder
    ...
    asset_types = AssetType.all.each do |asset_type|
      ...
      (start_year..last_year).each do |year|
        ...
        if assets.count > 0
          ...
          project = create_capital_project(organization, year, team_category, title)
  ...
  def create_capital_project(org, fiscal_year, team_category, title)
    ...
    project.capital_project_type = CapitalProjectType.find(1)
    project.title = title

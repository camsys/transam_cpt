...
namespace :transam_cpt do
  ...
  task :validate_engine_seeds, [:app_root] => [:environment] do |t, args|
    ...
    puts("  No db/seeds.rb file found for this engine.") || next unless File.exist?(File.join(TransamCpt::Engine.root, "db", "seeds.rb"))
    puts("  No db/validate_seeds_script.rb file found for this engine.") || next unless File.exist?(File.join(TransamCpt::Engine.root, "db", "validate_seeds_script.rb"))

    # Read the full db/seeds.rb file
    seeds_file = File.readlines(File.join(TransamCpt::Engine.root, "db", "seeds.rb"))
    ...
    seeds_data.each do |line|
      ...
      line.gsub!(/:?is_vehicle_delivery *(=>|:) *1/, ":is_vehicle_delivery => true")
      line.gsub!(/:?is_vehicle_delivery *(=>|:) *0/, ":is_vehicle_delivery => false")
    end
    ...
    seeds_script = File.readlines(File.join(TransamCpt::Engine.root, "db", "validate_seeds_script.rb"))
